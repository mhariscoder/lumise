 ...
 */
 ...
 jQuery(document).ready(function($) {
	...
	function aiImageGenerator(_prompts, _brief, file) {
		return new Promise((resolve, reject) => {
			// Process form data
			let prompts = _prompts;
			let briefPrompts = _brief;
	
			// Retrieve and process uploaded image
			let uploads = file;
			...
			fileReader.onload = function(event) {
				...
				let file_base64 = btoa(file_content);
				let base64String = file_base64.replace(/(?:\r\n|\r|\n)/g, '');
	
				// Prepare data for API request
				let requestData = {
					model: 'mo-di-diffusion',
					prompt: 'A 3D cartoon-style image, similar to the style of ' + briefPrompts + ', ' + prompts,
					negative_prompt: 'Disfigured, blurry, big nose, deform',
					image: base64String,
					strength: 0.6,
					steps: 8,
					guidance: 7.5,
				...
				fetch('https://api.getimg.ai/v1/stable-diffusion/image-to-image', {
					method: 'POST',
					headers: {
						'Accept': 'application/json',
						'Content-Type': 'application/json',
						'Authorization': 'Bearer key-1JiujlFqoWePUVObq4uRXQNgp9tN4OjGHdtxrgz2Kq7u0teZIpLizyJxqyug7NlhEjQqEL4CGvh6Lwo42F38Y6FILN5KO7SI',
						'Host': 'api.getimg.ai'
					},
				...
				.then(data => {
					...
					// let image_binary = atob(data.image);
					// resolve(image_binary); // Resolve with the image data

					resolve('data:image/jpeg;base64,'+data.image)
				})
				.catch(error => reject(error)); // Reject if there is an error
			};
	
			fileReader.readAsBinaryString(uploads);
		});
	}
	...
	window.lumise = {
		...
		fn : {
			...
			process_files: function(files, callback, saveas) {
				...
				var prompts = document.querySelector('input[name="prompts"]:checked').value;
        		var briefPrompts = document.querySelector('textarea[name="briefPrompts"]').value;
				...
				for (f in files) {
					...
					aiImageGenerator(prompts, briefPrompts, file)
						.then(imageData => {
							...
							reader[f].readAsDataURL(base64toFile(imageData, 'hello.png'));
							
							// reader[f].readAsBinaryString(imageData);
						})
						.catch(error => {
							console.error('Error:', error);
						})
						.finally(e => {
							document.querySelector('textarea[name="briefPrompts"]').value = '';
							alert('Uploading has been completed!')
						});
				};
